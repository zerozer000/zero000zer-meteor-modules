...
public class AntiAFKPlus extends Module {
    ...
    //Delay
    private final Setting<Boolean> afkdelaybool = sgDelay.add(new BoolSetting.Builder()
        .name("Delay")
        .description("Time after afk action will starts")
        .defaultValue(false)
        .build()
    );

    private final Setting<Integer> afkdelay = sgDelay.add(new IntSetting.Builder()
        .name("Delay")
        .description("Time after afk action will starts. (minutes)")
        .defaultValue(15)
        .min(0)
        .sliderMax(120)
        //.visible(sendMessages::get)
    ...
    public AntiAFKPlus() {
        super(AddonTemplate.CATEGORY, "anti-afk+", "Performs different actions to prevent getting kicked while AFK.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if(afkdelaybool.get() == true)
        {
            //delayed anti afk

        }
        else if (afkdelaybool.get() == false)
        {
            //based anti afk
